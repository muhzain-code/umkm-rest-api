...
type CategoryRepository interface {
	...
	FindAll(page, limit int, filter CategoryFilter) ([]model.Category, int64, error)
...
func (r *categoryRepository) FindAll(page, limit int, filter CategoryFilter) ([]model.Category, int64, error) {
	var categories []model.Category
	var total int64

	db := r.db.Model(&model.Category{})
	db = ApplyCategoryFilter(db, filter)
